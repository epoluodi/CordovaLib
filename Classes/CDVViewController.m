...
- (void)createprogress
{
    ...
    webprogressview = [[UIProgressView alloc] init];
    webprogressview.frame =CGRectMake(0, 58, self.view.bounds.size.width, 2);
    webprogressview.progress=0;
    
    webprogressview.progressTintColor = [UIColor greenColor];
...
-(void)startProcess
{
    ...
    webprogressview.alpha=1;
    catransition.delegate = self;
    [webprogressview.layer addAnimation:catransition forKey:nil];
    
    [webprogressview setProgress:0.6 animated:YES];
  
}

-(void)animationDidStop:(CAAnimation *)anim finished:(BOOL)flag
{
    timer = [NSTimer timerWithTimeInterval:0.5 target:self selector:@selector(timerprocess) userInfo:nil repeats:YES];
...
-(void)timerprocess
{
    ...
    if (webprogressview.progress >= 0.9)
    {
        return;
    }
    
    
    [webprogressview setProgress:webprogressview.progress+processoffet animated:YES];
    if (webprogressview.progress >= 0.8)
        processoffet = 0.01;
...
-(void)startProcess2
{
    ...
    CABasicAnimation *cabasicanimation = [CABasicAnimation animationWithKeyPath:@"opacity"];
    
    cabasicanimation.timeOffset=1.2;
    cabasicanimation.duration = 0.6;
    cabasicanimation.timingFunction=[CAMediaTimingFunction functionWithName:kCAMediaTimingFunctionEaseInEaseOut];
    cabasicanimation.fromValue = @1;
    cabasicanimation.toValue = @0;
    webprogressview.alpha=0;
    [webprogressview.layer addAnimation:cabasicanimation forKey:nil];
...
{
  NSLog(@"Error Url: %@", theWebView.request.URL);
    [CDVUserAgentUtil releaseLock:&_userAgentLockToken];
...
- (void)webView:(UIWebView *)sender runJavaScriptAlertPanelWithMessage:(NSString *)message initiatedByFrame:(CGRect *)frame {
    ...
    UIAlertView* alert = [[UIAlertView alloc] initWithTitle:@"提示信息" message:message delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil];
    [alert show];
